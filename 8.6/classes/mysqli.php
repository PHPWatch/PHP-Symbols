<?php

return array (
  'type' => 'class',
  'name' => 'mysqli',
  'meta' => 
  array (
    'type' => 'class',
    'name' => 'mysqli',
    'description' => '',
    'keywords' => 
    array (
    ),
    'added' => '0.0',
    'deprecated' => NULL,
    'removed' => NULL,
    'resources' => 
    array (
      0 => 
      array (
        'name' => 'mysqli interface (php.net)',
        'url' => 'https://www.php.net/manual/class.mysqli.php',
      ),
    ),
  ),
  'comment' => false,
  'namespace' => '',
  'constants' => 
  array (
  ),
  'methods' => 
  array (
    '__construct' => 
    array (
      'name' => '__construct',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'hostname' => 
        array (
          'position' => 0,
          'name' => 'hostname',
          'type' => '?string',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'username' => 
        array (
          'position' => 1,
          'name' => 'username',
          'type' => '?string',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'password' => 
        array (
          'position' => 2,
          'name' => 'password',
          'type' => '?string',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'database' => 
        array (
          'position' => 3,
          'name' => 'database',
          'type' => '?string',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'port' => 
        array (
          'position' => 4,
          'name' => 'port',
          'type' => '?int',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'socket' => 
        array (
          'position' => 5,
          'name' => 'socket',
          'type' => '?string',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'autocommit' => 
    array (
      'name' => 'autocommit',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'enable' => 
        array (
          'position' => 0,
          'name' => 'enable',
          'type' => 'bool',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'begin_transaction' => 
    array (
      'name' => 'begin_transaction',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'flags' => 
        array (
          'position' => 0,
          'name' => 'flags',
          'type' => 'int',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'name' => 
        array (
          'position' => 1,
          'name' => 'name',
          'type' => '?string',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'change_user' => 
    array (
      'name' => 'change_user',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'username' => 
        array (
          'position' => 0,
          'name' => 'username',
          'type' => 'string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'password' => 
        array (
          'position' => 1,
          'name' => 'password',
          'type' => 'string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'database' => 
        array (
          'position' => 2,
          'name' => 'database',
          'type' => '?string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'character_set_name' => 
    array (
      'name' => 'character_set_name',
      'class' => 'mysqli',
      'parameters' => 
      array (
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'close' => 
    array (
      'name' => 'close',
      'class' => 'mysqli',
      'parameters' => 
      array (
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'commit' => 
    array (
      'name' => 'commit',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'flags' => 
        array (
          'position' => 0,
          'name' => 'flags',
          'type' => 'int',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'name' => 
        array (
          'position' => 1,
          'name' => 'name',
          'type' => '?string',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'connect' => 
    array (
      'name' => 'connect',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'hostname' => 
        array (
          'position' => 0,
          'name' => 'hostname',
          'type' => '?string',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'username' => 
        array (
          'position' => 1,
          'name' => 'username',
          'type' => '?string',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'password' => 
        array (
          'position' => 2,
          'name' => 'password',
          'type' => '?string',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'database' => 
        array (
          'position' => 3,
          'name' => 'database',
          'type' => '?string',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'port' => 
        array (
          'position' => 4,
          'name' => 'port',
          'type' => '?int',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'socket' => 
        array (
          'position' => 5,
          'name' => 'socket',
          'type' => '?string',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'dump_debug_info' => 
    array (
      'name' => 'dump_debug_info',
      'class' => 'mysqli',
      'parameters' => 
      array (
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'debug' => 
    array (
      'name' => 'debug',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'options' => 
        array (
          'position' => 0,
          'name' => 'options',
          'type' => 'string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'get_charset' => 
    array (
      'name' => 'get_charset',
      'class' => 'mysqli',
      'parameters' => 
      array (
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'execute_query' => 
    array (
      'name' => 'execute_query',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'query' => 
        array (
          'position' => 0,
          'name' => 'query',
          'type' => 'string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'params' => 
        array (
          'position' => 1,
          'name' => 'params',
          'type' => '?array',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => 'mysqli_result|bool',
      'has_return_type' => true,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'get_client_info' => 
    array (
      'name' => 'get_client_info',
      'class' => 'mysqli',
      'parameters' => 
      array (
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'get_connection_stats' => 
    array (
      'name' => 'get_connection_stats',
      'class' => 'mysqli',
      'parameters' => 
      array (
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'get_server_info' => 
    array (
      'name' => 'get_server_info',
      'class' => 'mysqli',
      'parameters' => 
      array (
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'get_warnings' => 
    array (
      'name' => 'get_warnings',
      'class' => 'mysqli',
      'parameters' => 
      array (
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'init' => 
    array (
      'name' => 'init',
      'class' => 'mysqli',
      'parameters' => 
      array (
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'kill' => 
    array (
      'name' => 'kill',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'process_id' => 
        array (
          'position' => 0,
          'name' => 'process_id',
          'type' => 'int',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'multi_query' => 
    array (
      'name' => 'multi_query',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'query' => 
        array (
          'position' => 0,
          'name' => 'query',
          'type' => 'string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'more_results' => 
    array (
      'name' => 'more_results',
      'class' => 'mysqli',
      'parameters' => 
      array (
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'next_result' => 
    array (
      'name' => 'next_result',
      'class' => 'mysqli',
      'parameters' => 
      array (
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'ping' => 
    array (
      'name' => 'ping',
      'class' => 'mysqli',
      'parameters' => 
      array (
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'poll' => 
    array (
      'name' => 'poll',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'read' => 
        array (
          'position' => 0,
          'name' => 'read',
          'type' => '?array',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'error' => 
        array (
          'position' => 1,
          'name' => 'error',
          'type' => '?array',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'reject' => 
        array (
          'position' => 2,
          'name' => 'reject',
          'type' => 'array',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'seconds' => 
        array (
          'position' => 3,
          'name' => 'seconds',
          'type' => 'int',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'microseconds' => 
        array (
          'position' => 4,
          'name' => 'microseconds',
          'type' => 'int',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => true,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'prepare' => 
    array (
      'name' => 'prepare',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'query' => 
        array (
          'position' => 0,
          'name' => 'query',
          'type' => 'string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'query' => 
    array (
      'name' => 'query',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'query' => 
        array (
          'position' => 0,
          'name' => 'query',
          'type' => 'string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'result_mode' => 
        array (
          'position' => 1,
          'name' => 'result_mode',
          'type' => 'int',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => true,
          'default_value_constant' => 'MYSQLI_STORE_RESULT',
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'real_connect' => 
    array (
      'name' => 'real_connect',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'hostname' => 
        array (
          'position' => 0,
          'name' => 'hostname',
          'type' => '?string',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'username' => 
        array (
          'position' => 1,
          'name' => 'username',
          'type' => '?string',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'password' => 
        array (
          'position' => 2,
          'name' => 'password',
          'type' => '?string',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'database' => 
        array (
          'position' => 3,
          'name' => 'database',
          'type' => '?string',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'port' => 
        array (
          'position' => 4,
          'name' => 'port',
          'type' => '?int',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'socket' => 
        array (
          'position' => 5,
          'name' => 'socket',
          'type' => '?string',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'flags' => 
        array (
          'position' => 6,
          'name' => 'flags',
          'type' => 'int',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'real_escape_string' => 
    array (
      'name' => 'real_escape_string',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'string' => 
        array (
          'position' => 0,
          'name' => 'string',
          'type' => 'string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'reap_async_query' => 
    array (
      'name' => 'reap_async_query',
      'class' => 'mysqli',
      'parameters' => 
      array (
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'escape_string' => 
    array (
      'name' => 'escape_string',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'string' => 
        array (
          'position' => 0,
          'name' => 'string',
          'type' => 'string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'real_query' => 
    array (
      'name' => 'real_query',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'query' => 
        array (
          'position' => 0,
          'name' => 'query',
          'type' => 'string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'release_savepoint' => 
    array (
      'name' => 'release_savepoint',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'name' => 
        array (
          'position' => 0,
          'name' => 'name',
          'type' => 'string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'rollback' => 
    array (
      'name' => 'rollback',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'flags' => 
        array (
          'position' => 0,
          'name' => 'flags',
          'type' => 'int',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'name' => 
        array (
          'position' => 1,
          'name' => 'name',
          'type' => '?string',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'savepoint' => 
    array (
      'name' => 'savepoint',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'name' => 
        array (
          'position' => 0,
          'name' => 'name',
          'type' => 'string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'select_db' => 
    array (
      'name' => 'select_db',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'database' => 
        array (
          'position' => 0,
          'name' => 'database',
          'type' => 'string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'set_charset' => 
    array (
      'name' => 'set_charset',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'charset' => 
        array (
          'position' => 0,
          'name' => 'charset',
          'type' => 'string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'options' => 
    array (
      'name' => 'options',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'option' => 
        array (
          'position' => 0,
          'name' => 'option',
          'type' => 'int',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'value' => 
        array (
          'position' => 1,
          'name' => 'value',
          'type' => NULL,
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'set_opt' => 
    array (
      'name' => 'set_opt',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'option' => 
        array (
          'position' => 0,
          'name' => 'option',
          'type' => 'int',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'value' => 
        array (
          'position' => 1,
          'name' => 'value',
          'type' => NULL,
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'ssl_set' => 
    array (
      'name' => 'ssl_set',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'key' => 
        array (
          'position' => 0,
          'name' => 'key',
          'type' => '?string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'certificate' => 
        array (
          'position' => 1,
          'name' => 'certificate',
          'type' => '?string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'ca_certificate' => 
        array (
          'position' => 2,
          'name' => 'ca_certificate',
          'type' => '?string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'ca_path' => 
        array (
          'position' => 3,
          'name' => 'ca_path',
          'type' => '?string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
        'cipher_algos' => 
        array (
          'position' => 4,
          'name' => 'cipher_algos',
          'type' => '?string',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'stat' => 
    array (
      'name' => 'stat',
      'class' => 'mysqli',
      'parameters' => 
      array (
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'stmt_init' => 
    array (
      'name' => 'stmt_init',
      'class' => 'mysqli',
      'parameters' => 
      array (
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'store_result' => 
    array (
      'name' => 'store_result',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'mode' => 
        array (
          'position' => 0,
          'name' => 'mode',
          'type' => 'int',
          'is_optional' => true,
          'has_default_value' => true,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'thread_safe' => 
    array (
      'name' => 'thread_safe',
      'class' => 'mysqli',
      'parameters' => 
      array (
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'use_result' => 
    array (
      'name' => 'use_result',
      'class' => 'mysqli',
      'parameters' => 
      array (
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
    'refresh' => 
    array (
      'name' => 'refresh',
      'class' => 'mysqli',
      'parameters' => 
      array (
        'flags' => 
        array (
          'position' => 0,
          'name' => 'flags',
          'type' => 'int',
          'is_optional' => false,
          'has_default_value' => false,
          'has_default_value_constant' => false,
          'default_value_constant' => NULL,
        ),
      ),
      'return_type' => NULL,
      'has_return_type' => false,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
    ),
  ),
  'interfaces' => 
  array (
  ),
  'properties' => 
  array (
    'affected_rows' => 
    array (
      'name' => 'affected_rows',
      'class' => 'mysqli',
      'type' => 'string|int',
      'has_default_value' => false,
      'default_value' => NULL,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
      'is_promoted' => false,
    ),
    'client_info' => 
    array (
      'name' => 'client_info',
      'class' => 'mysqli',
      'type' => 'string',
      'has_default_value' => false,
      'default_value' => NULL,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
      'is_promoted' => false,
    ),
    'client_version' => 
    array (
      'name' => 'client_version',
      'class' => 'mysqli',
      'type' => 'int',
      'has_default_value' => false,
      'default_value' => NULL,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
      'is_promoted' => false,
    ),
    'connect_errno' => 
    array (
      'name' => 'connect_errno',
      'class' => 'mysqli',
      'type' => 'int',
      'has_default_value' => false,
      'default_value' => NULL,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
      'is_promoted' => false,
    ),
    'connect_error' => 
    array (
      'name' => 'connect_error',
      'class' => 'mysqli',
      'type' => '?string',
      'has_default_value' => false,
      'default_value' => NULL,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
      'is_promoted' => false,
    ),
    'errno' => 
    array (
      'name' => 'errno',
      'class' => 'mysqli',
      'type' => 'int',
      'has_default_value' => false,
      'default_value' => NULL,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
      'is_promoted' => false,
    ),
    'error' => 
    array (
      'name' => 'error',
      'class' => 'mysqli',
      'type' => 'string',
      'has_default_value' => false,
      'default_value' => NULL,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
      'is_promoted' => false,
    ),
    'error_list' => 
    array (
      'name' => 'error_list',
      'class' => 'mysqli',
      'type' => 'array',
      'has_default_value' => false,
      'default_value' => NULL,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
      'is_promoted' => false,
    ),
    'field_count' => 
    array (
      'name' => 'field_count',
      'class' => 'mysqli',
      'type' => 'int',
      'has_default_value' => false,
      'default_value' => NULL,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
      'is_promoted' => false,
    ),
    'host_info' => 
    array (
      'name' => 'host_info',
      'class' => 'mysqli',
      'type' => 'string',
      'has_default_value' => false,
      'default_value' => NULL,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
      'is_promoted' => false,
    ),
    'info' => 
    array (
      'name' => 'info',
      'class' => 'mysqli',
      'type' => '?string',
      'has_default_value' => false,
      'default_value' => NULL,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
      'is_promoted' => false,
    ),
    'insert_id' => 
    array (
      'name' => 'insert_id',
      'class' => 'mysqli',
      'type' => 'string|int',
      'has_default_value' => false,
      'default_value' => NULL,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
      'is_promoted' => false,
    ),
    'server_info' => 
    array (
      'name' => 'server_info',
      'class' => 'mysqli',
      'type' => 'string',
      'has_default_value' => false,
      'default_value' => NULL,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
      'is_promoted' => false,
    ),
    'server_version' => 
    array (
      'name' => 'server_version',
      'class' => 'mysqli',
      'type' => 'int',
      'has_default_value' => false,
      'default_value' => NULL,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
      'is_promoted' => false,
    ),
    'sqlstate' => 
    array (
      'name' => 'sqlstate',
      'class' => 'mysqli',
      'type' => 'string',
      'has_default_value' => false,
      'default_value' => NULL,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
      'is_promoted' => false,
    ),
    'protocol_version' => 
    array (
      'name' => 'protocol_version',
      'class' => 'mysqli',
      'type' => 'int',
      'has_default_value' => false,
      'default_value' => NULL,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
      'is_promoted' => false,
    ),
    'thread_id' => 
    array (
      'name' => 'thread_id',
      'class' => 'mysqli',
      'type' => 'int',
      'has_default_value' => false,
      'default_value' => NULL,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
      'is_promoted' => false,
    ),
    'warning_count' => 
    array (
      'name' => 'warning_count',
      'class' => 'mysqli',
      'type' => 'int',
      'has_default_value' => false,
      'default_value' => NULL,
      'is_static' => false,
      'is_public' => true,
      'is_protected' => false,
      'is_private' => false,
      'is_promoted' => false,
    ),
  ),
  'extension' => 'mysqli',
  'toString' => 'Class [ <internal:mysqli> class mysqli ] {

  - Constants [0] {
  }

  - Static properties [0] {
  }

  - Static methods [1] {
    Method [ <internal:mysqli> static public method poll ] {

      - Parameters [5] {
        Parameter #0 [ <required> ?array &$read ]
        Parameter #1 [ <required> ?array &$error ]
        Parameter #2 [ <required> array &$reject ]
        Parameter #3 [ <required> int $seconds ]
        Parameter #4 [ <optional> int $microseconds = 0 ]
      }
      - Tentative return [ int|false ]
    }
  }

  - Properties [18] {
    Property [ public string|int $affected_rows ]
    Property [ public string $client_info ]
    Property [ public int $client_version ]
    Property [ public int $connect_errno ]
    Property [ public ?string $connect_error ]
    Property [ public int $errno ]
    Property [ public string $error ]
    Property [ public array $error_list ]
    Property [ public int $field_count ]
    Property [ public string $host_info ]
    Property [ public ?string $info ]
    Property [ public string|int $insert_id ]
    Property [ public string $server_info ]
    Property [ public int $server_version ]
    Property [ public string $sqlstate ]
    Property [ public int $protocol_version ]
    Property [ public int $thread_id ]
    Property [ public int $warning_count ]
  }

  - Methods [43] {
    Method [ <internal:mysqli, ctor> public method __construct ] {

      - Parameters [6] {
        Parameter #0 [ <optional> ?string $hostname = null ]
        Parameter #1 [ <optional> ?string $username = null ]
        Parameter #2 [ <optional> ?string $password = null ]
        Parameter #3 [ <optional> ?string $database = null ]
        Parameter #4 [ <optional> ?int $port = null ]
        Parameter #5 [ <optional> ?string $socket = null ]
      }
    }

    Method [ <internal:mysqli> public method autocommit ] {

      - Parameters [1] {
        Parameter #0 [ <required> bool $enable ]
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method begin_transaction ] {

      - Parameters [2] {
        Parameter #0 [ <optional> int $flags = 0 ]
        Parameter #1 [ <optional> ?string $name = null ]
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method change_user ] {

      - Parameters [3] {
        Parameter #0 [ <required> string $username ]
        Parameter #1 [ <required> string $password ]
        Parameter #2 [ <required> ?string $database ]
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method character_set_name ] {

      - Parameters [0] {
      }
      - Tentative return [ string ]
    }

    Method [ <internal:mysqli> public method close ] {

      - Parameters [0] {
      }
      - Tentative return [ true ]
    }

    Method [ <internal:mysqli> public method commit ] {

      - Parameters [2] {
        Parameter #0 [ <optional> int $flags = 0 ]
        Parameter #1 [ <optional> ?string $name = null ]
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method connect ] {

      - Parameters [6] {
        Parameter #0 [ <optional> ?string $hostname = null ]
        Parameter #1 [ <optional> ?string $username = null ]
        Parameter #2 [ <optional> ?string $password = null ]
        Parameter #3 [ <optional> ?string $database = null ]
        Parameter #4 [ <optional> ?int $port = null ]
        Parameter #5 [ <optional> ?string $socket = null ]
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method dump_debug_info ] {

      - Parameters [0] {
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method debug ] {

      - Parameters [1] {
        Parameter #0 [ <required> string $options ]
      }
      - Tentative return [ true ]
    }

    Method [ <internal:mysqli> public method get_charset ] {

      - Parameters [0] {
      }
      - Tentative return [ ?object ]
    }

    Method [ <internal:mysqli> public method execute_query ] {

      - Parameters [2] {
        Parameter #0 [ <required> string $query ]
        Parameter #1 [ <optional> ?array $params = null ]
      }
      - Return [ mysqli_result|bool ]
    }

    Method [ <internal, deprecated:mysqli> public method get_client_info ] {

      - Parameters [0] {
      }
      - Tentative return [ string ]
    }

    Method [ <internal:mysqli> public method get_connection_stats ] {

      - Parameters [0] {
      }
      - Tentative return [ array ]
    }

    Method [ <internal:mysqli> public method get_server_info ] {

      - Parameters [0] {
      }
      - Tentative return [ string ]
    }

    Method [ <internal:mysqli> public method get_warnings ] {

      - Parameters [0] {
      }
      - Tentative return [ mysqli_warning|false ]
    }

    Method [ <internal, deprecated:mysqli> public method init ] {

      - Parameters [0] {
      }
    }

    Method [ <internal, deprecated:mysqli> public method kill ] {

      - Parameters [1] {
        Parameter #0 [ <required> int $process_id ]
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method multi_query ] {

      - Parameters [1] {
        Parameter #0 [ <required> string $query ]
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method more_results ] {

      - Parameters [0] {
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method next_result ] {

      - Parameters [0] {
      }
      - Tentative return [ bool ]
    }

    Method [ <internal, deprecated:mysqli> public method ping ] {

      - Parameters [0] {
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method prepare ] {

      - Parameters [1] {
        Parameter #0 [ <required> string $query ]
      }
      - Tentative return [ mysqli_stmt|false ]
    }

    Method [ <internal:mysqli> public method query ] {

      - Parameters [2] {
        Parameter #0 [ <required> string $query ]
        Parameter #1 [ <optional> int $result_mode = MYSQLI_STORE_RESULT ]
      }
      - Tentative return [ mysqli_result|bool ]
    }

    Method [ <internal:mysqli> public method real_connect ] {

      - Parameters [7] {
        Parameter #0 [ <optional> ?string $hostname = null ]
        Parameter #1 [ <optional> ?string $username = null ]
        Parameter #2 [ <optional> ?string $password = null ]
        Parameter #3 [ <optional> ?string $database = null ]
        Parameter #4 [ <optional> ?int $port = null ]
        Parameter #5 [ <optional> ?string $socket = null ]
        Parameter #6 [ <optional> int $flags = 0 ]
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method real_escape_string ] {

      - Parameters [1] {
        Parameter #0 [ <required> string $string ]
      }
      - Tentative return [ string ]
    }

    Method [ <internal:mysqli> public method reap_async_query ] {

      - Parameters [0] {
      }
      - Tentative return [ mysqli_result|bool ]
    }

    Method [ <internal:mysqli> public method escape_string ] {

      - Parameters [1] {
        Parameter #0 [ <required> string $string ]
      }
      - Tentative return [ string ]
    }

    Method [ <internal:mysqli> public method real_query ] {

      - Parameters [1] {
        Parameter #0 [ <required> string $query ]
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method release_savepoint ] {

      - Parameters [1] {
        Parameter #0 [ <required> string $name ]
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method rollback ] {

      - Parameters [2] {
        Parameter #0 [ <optional> int $flags = 0 ]
        Parameter #1 [ <optional> ?string $name = null ]
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method savepoint ] {

      - Parameters [1] {
        Parameter #0 [ <required> string $name ]
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method select_db ] {

      - Parameters [1] {
        Parameter #0 [ <required> string $database ]
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method set_charset ] {

      - Parameters [1] {
        Parameter #0 [ <required> string $charset ]
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method options ] {

      - Parameters [2] {
        Parameter #0 [ <required> int $option ]
        Parameter #1 [ <required> $value ]
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method set_opt ] {

      - Parameters [2] {
        Parameter #0 [ <required> int $option ]
        Parameter #1 [ <required> $value ]
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method ssl_set ] {

      - Parameters [5] {
        Parameter #0 [ <required> ?string $key ]
        Parameter #1 [ <required> ?string $certificate ]
        Parameter #2 [ <required> ?string $ca_certificate ]
        Parameter #3 [ <required> ?string $ca_path ]
        Parameter #4 [ <required> ?string $cipher_algos ]
      }
      - Tentative return [ true ]
    }

    Method [ <internal:mysqli> public method stat ] {

      - Parameters [0] {
      }
      - Tentative return [ string|false ]
    }

    Method [ <internal:mysqli> public method stmt_init ] {

      - Parameters [0] {
      }
      - Tentative return [ mysqli_stmt|false ]
    }

    Method [ <internal:mysqli> public method store_result ] {

      - Parameters [1] {
        Parameter #0 [ <optional> int $mode = 0 ]
      }
      - Tentative return [ mysqli_result|false ]
    }

    Method [ <internal:mysqli> public method thread_safe ] {

      - Parameters [0] {
      }
      - Tentative return [ bool ]
    }

    Method [ <internal:mysqli> public method use_result ] {

      - Parameters [0] {
      }
      - Tentative return [ mysqli_result|false ]
    }

    Method [ <internal, deprecated:mysqli> public method refresh ] {

      - Parameters [1] {
        Parameter #0 [ <required> int $flags ]
      }
      - Tentative return [ bool ]
    }
  }
}
',
  'modifiers' => 0,
  'defaultProperties' => 
  array (
  ),
  'staticProperties' => 
  array (
  ),
  'traits' => 
  array (
  ),
  'is_abstract' => false,
  'is_anonymous' => false,
  'is_cloneable' => false,
  'is_final' => false,
  'is_instantiable' => true,
  'is_read_only' => false,
);
